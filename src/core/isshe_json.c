...
// static functions
static isshe_uchar_t decimal_point_get(isshe_void_t);
static isshe_bool_t is_equal_double(double a, double b);
...
static isshe_bool_t isshe_json_parse_value(isshe_json_t * const item,
    isshe_json_parse_buffer_t * const input_buffer);
static isshe_size_t number_print_length_get(isshe_double_t num,
    isshe_json_print_buffer_t *pbuf);
static isshe_size_t string_print_length_get(const isshe_uchar_t *str,
    isshe_json_print_buffer_t *pbuf);
static isshe_size_t array_print_length_get(
    const isshe_json_t * const item,
    isshe_json_print_buffer_t * const pbuf);
static isshe_size_t object_print_length_get(
    const isshe_json_t * const item,
    isshe_json_print_buffer_t * const pbuf);
static isshe_size_t print_length_get(
    const isshe_json_t * const item,
    isshe_json_print_buffer_t * const pbuf);
static isshe_size_t print_length_common(
...
decimal_point_get(isshe_void_t)
{
#ifdef ENABLE_LOCALES
    ...
    isshe_json_parse_buffer_t * const input_buffer)
{
    ...
    isshe_uchar_t decimal_point = decimal_point_get();
    isshe_size_t i = 0;
...
static isshe_size_t
number_print_length_get(isshe_double_t num,
    isshe_json_print_buffer_t *pbuf)
{
    isshe_int_t len = 0;
...
static isshe_size_t
string_print_length_get(const isshe_uchar_t *str,
    isshe_json_print_buffer_t *pbuf)
{
    isshe_size_t len = 0;
    isshe_size_t i = 0;
...
static isshe_size_t
array_print_length_get(const isshe_json_t * const item,
    isshe_json_print_buffer_t *pbuf)
{
    isshe_size_t len = 0;
    ...
    len += 1;   // '['
    while(current) {
        len += print_length_get(current, pbuf);
...
static isshe_size_t
object_print_length_get(const isshe_json_t * const item,
    isshe_json_print_buffer_t *pbuf)
{
    isshe_size_t len = 0;
    isshe_json_t *current = item->child;
    ...
    while (current) {
        ...
        len += rc;
        len += format ? 2 : 1;  // ":\t"、":"
        rc = print_length_get(current, pbuf);
        if (rc == 0) {
            return 0;
        }
        len += rc;
...
static isshe_size_t
print_length_get(const isshe_json_t * const item,
    isshe_json_print_buffer_t *pbuf)
{
    isshe_size_t len = 0;
    ...
    case ISSHE_JSON_NUMBER:
        len += number_print_length_get(item->vdouble, pbuf);
        break;
    case ISSHE_JSON_STRING:
        len += string_print_length_get((isshe_uchar_t *)item->vstring, pbuf);
        break;
    case ISSHE_JSON_ARRAY:
        len += array_print_length_get(item, pbuf);
        break;
    case ISSHE_JSON_OBJECT:
        len += object_print_length_get(item, pbuf);
        break;
    case ISSHE_JSON_RAW:
    ...
    isshe_bool_t format)
{
    ...
    len = print_length_get(item, pbuf) + 1;
...
{
    return print_length_common(item, ISSHE_FALSE);
}

isshe_size_t
isshe_json_print_format_length(const isshe_json_t * const item)
{
    return print_length_common(item, ISSHE_TRUE);
}
...
static isshe_int_t
json_print(const isshe_json_t * const item, isshe_bool_t format)
{
    isshe_json_print_buffer_t pbuf[1];
    isshe_size_t len = 0;

    len = print_length_common(item, format);
    if (len == 0) {
        return ISSHE_ERROR;
    }
    printf("---isshe---: json_print: len = %zu\n", len);
    isshe_memzero(pbuf, sizeof(pbuf));
...
isshe_int_t
isshe_json_print(const isshe_json_t * const item)
{
    return json_print(item, ISSHE_FALSE);
}

isshe_int_t
isshe_json_print_format(const isshe_json_t * const item)
{
    return json_print(item, ISSHE_TRUE);
}
